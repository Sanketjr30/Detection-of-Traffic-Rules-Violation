...
class EuclideanDistTracker:
        ...
        for rect in objects_rect:
            ...
            for id, pt in self.center_points.items():
                ...
                if dist < 70:# traffic4
                    ...
                    # START TIMER
                    # p = y+h # for source
                    if(y >= 410 and y <= 430):  #for traffic4
                    # if (p >= 180 and p <= 200): # for source
                        self.s1[0, id] = time.time()

                    # STOP TIMER and FIND DIFFERENCE
                    if (y>=235 and y<=255):     #for traffic4
                    # if (p >= 410 and y <= 430): #for source
                        self.s2[0, id] = time.time()
                        self.s[0, id] = self.s2[0, id] - self.s1[0, id]

                    # CAPTURE FLAG
                    if(y < 235):    #for traffic4
                    # if (y < 430):   #for source
                        self.f[id] = 1
    ...
    def getsp(self, id):
        if (self.s[0, id] != 0):
            #  s = 80 / self.s[0, id]     #for source
            s = 214.15 / self.s[0, id] #for traffic4
        else:
            s = 0
    ...
    def capture(self, img, x, y, h, w, sp, id):
        if (self.capf[id] == 0):
            self.capf[id] = 1
            self.f[id] = 0
            crop_img = img[y - 5:y + h + 5, x - 5:x + w + 5] #for traffic4
            # crop_img = img[y:y + h, x:x + w] for source
